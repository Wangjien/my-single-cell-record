...
步骤 a：模糊聚类（Soft assign cells to clusters）

把所有细胞软分配到多个聚类中（不是 “非此即彼” 的硬分配），同时通过惩罚项，让每个聚类内尽可能包含不同数据集的细胞（保证聚类的 “跨批次多样性”）。
图 a 中能看到：不同颜色（数据集）的细胞被分到了同一个聚类里（比如 Cluster 1 包含多种颜色）。
步骤 b：计算聚类中心（Get cluster centroids）

对每个聚类，计算全局中心（整个聚类的中心），同时计算该聚类在每个数据集下的中心（批次特异性中心）。
图 b 中能看到：每个聚类（如 Cluster 1）出现了 “全局中心” 和 “不同颜色对应的批次中心”。
步骤 c：计算校正因子（Get dataset correction factors）

在每个聚类内，根据 “全局中心” 和 “批次中心” 的差异，计算每个数据集的校正因子（用于消除该批次在这个聚类中的偏移）。
图 c 中箭头表示：不同数据集的细胞会向全局中心的方向被校正。
步骤 d：校正细胞位置（Move cells based on soft cluster membership）

利用步骤 a 中细胞的 “软聚类分配权重”，对每个细胞应用多个聚类校正因子的线性组合（因为细胞被分配到多个聚类），最终调整细胞的位置。
图 d 中能看到：同一细胞类型（同形状）的不同颜色（不同数据集）细胞，位置更聚集了（批次效应被校正）。
核心逻辑

Harmony 通过 “模糊聚类→计算中心→算校正因子→校正细胞” 的迭代，逐渐削弱 “聚类分配” 与 “数据集（批次）” 的关联，最终让相同细胞类型（同形状）的细胞，不管来自哪个数据集（什么颜色），都能聚在一起（消除批次效应）。
